...
enum SystemMode {
  MODE_IDLE,
  MODE_A1, MODE_A2, MODE_A3, MODE_A4,
  MODE_B1, MODE_B2, MODE_B3, MODE_B4,
  MODE_C1, MODE_C2, MODE_C3, MODE_C4,
  MODE_D1, MODE_D2, MODE_D3, MODE_D4
};

const int TOTAL_MODES = 16;
...
void showSplashScreen() {
  lcd.clear();
  lcd.setCursor(5, 0); lcd.print("Timer Multi");
  lcd.setCursor(5, 1); lcd.print("CONTROLLER");
  lcd.setCursor(2, 3); lcd.print("Versi : 16 waktu");
  delay(2000);
  lcd.clear();
  lcd.setCursor(3, 0); lcd.print("Build BY AGUS F");
  lcd.setCursor(4, 2); lcd.print("15 JULI 2025");
  delay(2000);
}
...
void showAllModeTimes() {
  ...
  // Baris 1–3: Grup A–D (per halaman 3 grup)
  for (int i = 0; i < 3; i++) {
    int groupIndex = scrollOffset * 3 + i;
    if (groupIndex >= 4) break;
    ...
    for (int j = 0; j < 4; j++) {
      int index = groupIndex * 4 + j;
      float detik = modeTimes[index] / 1000.0;

      // Format nilai: 4 karakter lebar total, contoh: " 3.0", "10.0"
      lcd.print(" ");
      if (detik < 10) lcd.print("");  // Tambah spasi untuk angka <10 agar rata
      lcd.print(detik, 1);             // Cetak 1 angka desimal
    }
  }
...
void handleKeypadInput(char key) {
  if (key == '*' && !inputMode) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("  Pilih Mode A1-D4");
    lcd.setCursor(0, 2);
    lcd.print("Pencet A,B,C atau D");
    lcd.setCursor(0, 3);
    lcd.print("lalu tekan 1,2,3,4");
    ...
    lcd.setCursor(0, 1);
    lcd.print("Tekan 1-4");
  } else if (inputMode && !waitingForTime && key >= '1' && key <= '4') {
    int group = currentGroup - 'A';
    int number = key - '1';
    selectedModeToSet = (SystemMode)(group * 4 + number + 1);
    waitingForTime = true;
    inputValue = "";
    tempValue = 0;
    lcd.clear();
...
void saveInput() {
  if (selectedModeToSet >= MODE_A1 && selectedModeToSet <= MODE_D4) {
...
void resetAll() {
  ...
  digitalWrite(relayPin, HIGH);
  lcd.clear();
  lcd.setCursor(3, 1); lcd.print("Reset Berhasil");
  delay(1000);
  showMainScreen();
}
